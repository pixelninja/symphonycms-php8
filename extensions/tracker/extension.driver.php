...
class Extension_Tracker extends Extension
{
    ...
    public function parseEventSave($context)
    {
        if ($this->validateSection($context['entry']->get('section_id'))) {
            ...
            if (isset(Frontend::instance()->Author) && !is_null(Frontend::instance()->Author)) {
    ...
    public function parsePreferencesSave($context)
    {
        if ($this->validateUser() && $this->validateElement('preferences')) {
            ...
            $context['settings']['maintenance_mode'] = $context['settings']['maintenance_mode'] ?? array();
            $context['settings']['maintenance_mode']['enabled'] = $context['settings']['maintenance_mode']['enabled'] ?? 'no';
    ...
    public function dashboardPanelOptions($context)
    {
        ...
        $config['filter_string'] = $config['filter_string'] ?? null;
    ...
    public function renderPanel($context)
    {
        ...
        switch ($context['type']) {
            ...
            case 'tracker_activity':
                ...
                if (isset($config['filter_string']) && !empty($config['filter_string'])) {

                    var_dump(explode(':', $config['filter_string'], 2));
                    exit;
